...
- **No existe fila `VEP RETENCION`** (se agregó el 2026-09-11 y se retiró el 2026-09-12): las retenciones
  a empresas llegan como paquete de liquidación que nombra a la empresa (ver `LIQ_RETENCION`).
### Archivos clave
...
- `"VEP"` / `"VEP_RETENCION"` / `"VEP_MIXTO"` → **Volante Electrónico de Pago de ARCA** (2026-09-03, tipos 2026-09-11): el cupón con el que el consorcio paga las cargas sociales de su encargado **o ingresa retenciones a un tercero**. Se detecta por los marcadores del encabezado en los **primeros 200 caracteres** — es lo único que lo separa de un F931, que trae su propio VEP en la página 2. Va ANTES de la regla del `931` (un VEP de SICOSS no imprime ese número).
  > **Tipos de VEP (2026-09-11).** Los renglones traen códigos de impuesto ARCA y son lo ÚNICO que dice qué paga el cupón (`Tipo de Pago` / `Descripción Reducida` / `Concepto` son idénticos entre un VEP de retenciones y la ART del encargado). `classifyVep` en `lib/vepExtraction.ts`, regex, 0 tokens: **351/301/352/302/312/28 = encargado → `VEP`** (proveedor `ARCA EMPLEADO` por nombre); **217/767/216/353 = retención → `VEP_RETENCION`**: el cupón **suelto** no dice a quién se le retuvo (trae el CUIT del consorcio), así que `vepReviewGate` lo manda a **Revisión** `[VEP RETENCION SUELTO - SUBIR LIQUIDACION COMPLETA]` sin IA; la retención se carga desde el paquete (`LIQ_RETENCION`). Mixto o códigos desconocidos → `VEP_MIXTO` → Revisión `[VEP MIXTO]` / `[VEP SIN CLASIFICAR]`. Sin códigos legibles → `VEP`. Ni `VEP_RETENCION` ni `LIQ_RETENCION` están en `usesConsortiumCuit`.
...
| Liquidación de retenciones (paquete) | **sin prompt**: `lib/liqRetencion.ts` por regex; `buildVepPrompt()` sólo si el parser falla |
